...
C_ChatInfo.RegisterAddonMessagePrefix(KEYS_PREFIX)
...
aura_env.handleEncounterClientMessage = function(addonMessage)
    ...
    if data.chat then
        SendChatMessage(data.chat.message, data.chat.channel)
    end
...
aura_env.isValidAddonMessage = function(prefix, channel, sender)
    if not (prefix == CLIENT_PREFIX or prefix == KEYS_PREFIX or prefix == VERSION_PREFIX) then
    ...
    return (channel == "PARTY" or channel == "RAID" or channel == "WHISPER") and (fromSelf or UnitIsGroupLeader(sender))
end
...
aura_env.processAddonMessage = function(prefix, message, sender)
    ...
    if prefix == KEYS_PREFIX then
        aura_env.printMythicPlusKey("RAID")
    end
...
aura_env.printMythicPlusKey = function(channel)
    for bag = 0, NUM_BAG_SLOTS do
        for slot = 1, C_Container.GetContainerNumSlots(bag) do
            if C_Container.GetContainerItemID(bag, slot) == KEY_ITEM_ID then
                SendChatMessage(C_Container.GetContainerItemLink(bag, slot), channel)
                return
            end
        end
    end
...
-- Allow overridding who the master looter is ....

if RCLootCouncil then
    if aura_env.config.masterLooter then
        local masterLooterName = aura_env.config.masterLooter
    ...
    elseif RCLootCouncil._GetML then
        RCLootCouncil.GetML = RCLootCouncil._GetML
    end
