...
trigger = function(e, ...)
                ...
                if aura_env.casts <= 3 then
                    for i = 1, 12 do
                        ...
                        WeakAuras.ScanEvents(
                            "NS_OVINAX_EGG",
                            i,
                            aura_env.type[aura_env.phase][i],
                            duration,
                            type,
                            aura_env.phase,
                            num
        ...
        if spellID == aura_env.debuff then
            ...
            if not aura_env.last or aura_env.last < now - 5 then
                ...
                if aura_env.casts <= 2 then
                    aura_env.next = C_Timer.NewTimer(20, function()
                        WeakAuras.ScanEvents("NS_OVINAX_NEXT", 50, aura_env.id)
                    end)
                end
            end
            ...
            if unit and UnitExists(unit) then
                ...
                local spec = (aura_env.specs and aura_env.specs[unit])
                    or (NSAPI and NSAPI:GetSpecs(unit))
                    or WeakAuras.SpecForUnit(unit)
                local prio = spec and aura_env.spec and aura_env.spec[spec] or 0
                if prio == 0 then
                    print(
                        "no spec information found for:",
                        WA_ClassColorName(destName),
                        "You should probably reload/relog, can also be caused by ignore list."
                    )
    ...
    elseif e == "NS_ASSIGN_EVENT" and aura_env.id == ... then
        ...
        table.sort(aura_env.affected, function(a, b)
            ...
            if aura_env.prio[1] or aura_env.prio[2] or aura_env.prio[3] or aura_env.prio[4] then -- Hard assign certain players specified through the note
                for k = 1, 4 do
                    ...
                    if unit and UnitExists(unit) and aura_env.affected[i] and UnitExists(aura_env.affected[i][1]) then
                        if
                            UnitIsUnit(aura_env.affected[i][1], unit) or UnitIsUnit(aura_env.affected[i - 1][1], unit)
                        then
        ...
        for i, v in ipairs(aura_env.affected) do
            if UnitIsUnit(v[1], "player") then
                ...
                partner = aura_env.affected[partner] and aura_env.affected[partner][1]
                    or (aura_env.multiplier > 1 and "???")
                    or ""
        ...
        if aura_env.config.enabled then -- This is only relevant to the map display
            for i = 1, math.floor(#aura_env.affected / aura_env.multiplier) do
                ...
                WeakAuras.ScanEvents(
                    "NS_OVINAX_EGG",
                    type,
                    aura_env.type[aura_env.phase][type],
                    duration,
                    status,
                    aura_env.phase,
                    i
