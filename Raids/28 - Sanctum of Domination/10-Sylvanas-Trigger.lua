function (event, ...)
    ...
    local function AssignChain (player, position, stacks)
        ...
        if set == 2 and position == 'RANGED' and stacks < 2 then return end
        -- Do not assign Rolanor to right side on set 1
        if player == 'Rolanor' and set == 1 and count >= 4 then return end
        -- Do not assign Graggars/Bpaptu to right side on set 2
        if (player == 'Graggars' or player == 'Bpaptu') and set == 2 and count >= 4 then return end
        -- Do not assign Saeyra to right side on set 1 or 3
        if player == 'Saeyra' and (set == 1 or set == 3) and count >= 4 then return end
        -- Do not assign Arimist on set 3 to either side
        if player == 'Arimist' and set == 3 then return end
        ...
        if aura_env.chain_count <= 4 then
            ...
            C_Timer.After(3, function () Emit('c=YELL LEFT', player) end)
        elseif aura_env.chain_count <= 8 then
            print('RIGHT:', player)
            Emit('m1=>> RIGHT (2) >>;c=YELL RIGHT;s=bikehorn;d=7', player)
            C_Timer.After(3, function () Emit('c=YELL RIGHT', player) end)
        end
    end

    local function AssignChains ()
        if WA_GetUnitBuff('boss1', 350857) then return end -- Banshee Shroud = final chains
    ...
    if event == 'COMBAT_LOG_EVENT_UNFILTERED' then
        ...
        if subevent == 'SPELL_AURA_APPLIED' and spellID == 351451 then -- Curse of Lethargy
            for name, class in IterateActiveGroup() do
                if (class == 'DRUID' or class == 'SHAMAN' or class == 'MAGE' or class == 'PALADIN' or class == 'MONK') then
