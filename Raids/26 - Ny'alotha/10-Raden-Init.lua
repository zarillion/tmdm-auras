...
local function Emit(msg, t)
    SAM("TMDM_ECWAv1", msg, "WHISPER", t)
end
local function EmitRaid(msg)
    SAM("TMDM_ECWAv1", msg, "RAID")
end
...
aura_env.GetRoles = function()
    ...
    local MELEE_SWAPS =
        { "HUNTER", "DRUID", "SHAMAN", "DEMONHUNTER", "MONK", "WARRIOR", "ROGUE", "PALADIN", "DEATHKNIGHT" }
    ...
    if GetTime() - aura_env.lastUpdate < 0.25 then
        if not auto then
            ...
            C_Timer.After(0.5, function()
                aura_env.UpdateTeamDisplays(true)
            end)
        end
        return
    end
