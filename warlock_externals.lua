function (event, ...)
    ...
    if event == 'COMBAT_LOG_EVENT_UNFILTERED' then
        ...
        if sourceName ~= UnitName('player') then return end -- ignore other's casts

        if subevent == 'SPELL_CAST_SUCCESS' then
            local spec = GetSpecialization()
            if spec == 1 then CheckAfflictionCast(spellID)
            elseif spec == 2 then CheckDemonologyCast(spellID)
            elseif spec == 3 then CheckDestructionCast(spellID) end
        end
    end
end
