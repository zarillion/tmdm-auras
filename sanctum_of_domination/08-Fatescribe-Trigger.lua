function (event, ...)
    ...
    local function UnitPercent (unit)
        ...
        if cur > 0 and max > 0 then return cur / max end
        return 1
    end
    ...
    local function GetRole (player)
        for i = 1, 40 do
            ...
            if name == player then
                ...
                if UnitIsGroupLeader(name) and aura_env.affset < 3 and role == 'DAMAGER' then return 'HEALER' end
                return role
            end
        end
        return 'UNKNOWN'
    ...
    local function AssignRune (player, position)
        if position == 0 then
            ...
            Emit('m1='..markrt..' RUNE DUTY BIATCH ('..position..') '..markrt..';m3=(>^.(>O.o)>;d=30;s=moan', player)
        end
    end
    ...
    if event == 'COMBAT_LOG_EVENT_UNFILTERED' then
        ...
        if boss and boss < .74 and not aura_env.p1StopCalled then
            EmitRaid('m=|cFFFF0000KILL THE ADD YOU APES|r;s=bikehorn')
            aura_env.p1StopCalled = true
        end
        if boss and boss < .43 and not aura_env.p2StopCalled then
        ...
        if subevent == 'SPELL_AURA_APPLIED' and spellID == 354964 then -- Runic Affinity
            ...
            if #aura_env.debuffs == 1 then C_Timer.After(1, AssignRunes) end
        end
